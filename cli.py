"""
Command Line Interface for Crispr Offtarget Scanner.
"""
import argparse
import csv
import json
...
# Ensure AUDIT_SECRET_KEY is set for CLI operations
if not os.getenv("AUDIT_SECRET_KEY"):
    os.environ["AUDIT_SECRET_KEY"] = "cli-session-key-" + os.urandom(16).hex()

supervisor = SystemSupervisor(model_provider="mock")


def main(argv=None):
    parser = argparse.ArgumentParser(prog="crispr-offtarget-scanner", description="Crispr Offtarget Scanner")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  CRISPR OFFTARGET SCANNER")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Crispr Offtarget Scanner Supervisor]:\n{ans}\n")
        return 0
    ...
    if args.command == "batch":
        ...
        for r in rows:
            try:
                ...
                print(f"Warning: Skipping invalid row {r}: {e}", file=sys.stderr)
                continue
            dossier = supervisor.process_task(payload)
            row_dict = dict(r)
    ...
    if args.command == "serve":
        ...
        print(f"Starting Crispr Offtarget Scanner API server on http://{args.host}:{args.port}")
